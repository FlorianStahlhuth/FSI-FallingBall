/**
 ...
 E-mail: florian.stahlhuth@stud.uni-hannover.de 

 Date: Feb 17, 2026
 This code is based on the version deal.II.9.6.2.

 This code is a modification of the
 fluid-structure-interaction code by Thomas Wick:
...
{    
  ...
  template <int dim> 
    inline
    double 
    get_p (unsigned int q,
	    std::vector<Vector<double> > old_solution_values)
    {
      double tmp;
      tmp =  old_solution_values[q](dim+dim);
      return tmp;      
    }
 ...
 template <int dim> 
  ...
  template <int dim> 
    inline
    Tensor<2,dim> 
    get_grad_u_LinU (const Tensor<2,dim> phi_i_grads_u)	 
    {     
        Tensor<2,dim> tmp;		 
...
// subsequent Newton steps all Dirichlet values for the Newton update have to be equal zero.
...
// The last four functions provide the framework to compute 
// functional values of interest. For the given fluid-structure
// interaction problem, we compute at each time step the pressure at
// the bottom, the distance between the ball and the lower boundary and
// the minimal value of the determinant of the deformation gradient J.
// With these values we can calculate all other quantities of interest.  
template <int dim>
...
private:
  ...
  void compute_functional_values (const double time);
  void compute_minimal_dist_J();
  void compute_v_star();
  ...
  double min_dist = 0.039;
  double old_timestep_dist = 0.039;
  double dist_to_bottom = 0.039;
  double max_pressure = 0.0;
  double t0 = 0.0;
  double t_star = 0.0;
  double t0_plus_t_star = 0.0;
  double v_star = 0.0;
  double t_cont = 0.0;
  double t_jump = 0.0;
  double h_jump = 0.0;
  double av_newton_steps = 0.0;
  double global_min_J = 1;
};
...
// come from a paramter file. The reader is invited to change 
// these values to obtain other results.
template <int dim>
...
// Fluid parameters
  ...
  force_fluid_x = force_structure_x;
  force_fluid_y = force_structure_y;
  ...
  // In the following, we read a *.msh grid from a file.
...
// mesh cells with the material id == 10 and the structure
// equations on cells with the material id == 11. 
//
// Additionally, we compute the interface integral resulting
// from partial integration of the MMPDE. Interface cells are
// identified by having a neighbor cell with a different 
// material id.
//
        ...
        phi_i_u[k]       = fe_face_values[displacements].value (k, q);	
          ...
          const Tensor<2,dim>  grad_u = ALE_Transformations
			...
			::get_J<dim> (F);
		      ...
		      for (unsigned int i=0; i<dofs_per_cell; ++i)
			{
			  const Tensor<2,dim> grad_u_LinU = ALE_Transformations
			    ::get_grad_u_LinU<dim> (phi_i_grads_u[i]);
			  
			  const double J_LinU = ALE_Transformations
			    ::get_J_LinU<dim> (q, old_solution_face_grads, phi_i_grads_u[i]);

			  const Tensor<1,dim> neumann_value
			    = (((alpha_u / J) * grad_u_LinU 
          - alpha_u * J_LinU / (J*J) * grad_u
			  ...
			  for (unsigned int j=0; j<dofs_per_cell; ++j)
				...
				{
				  local_matrix(j,i) -= (neumann_value * phi_i_u[j]) * fe_face_values.JxW(q);
				}
			      // end j    
			    } 
			  // end i
...
// In this function we assemble the semi-linear form 
...
FSI_ALE_Problem<dim>::assemble_system_rhs ()
{
  ...
  for (const auto &cell : dof_handler.active_cell_iterators()) 
    { 
      ...
      // Again, material_id == 10 corresponds to 
      ...
      if (cell->material_id() == 10)
	{
	  for (unsigned int q=0; q<n_q_points; ++q)
		...
		(density_fluid * old_timestep_J * 
		 (old_timestep_grad_v * old_timestep_F_Inverse * old_timestep_v));
	      ...
	      fluid_force.clear();
	      fluid_force[0] = J * density_fluid * force_fluid_x;
	      fluid_force[1] = J * density_fluid * force_fluid_y;
	      
	      Tensor<1,dim> old_timestep_fluid_force;
	      old_timestep_fluid_force.clear();
	      old_timestep_fluid_force[0] = old_timestep_J * density_fluid * force_fluid_x;
	      old_timestep_fluid_force[1] = old_timestep_J * density_fluid * force_fluid_y;
	  ...
	  // we have to integrate some terms on the interface:
		  ...
		  for (unsigned int q=0; q<n_face_q_points; ++q)
           ...
           const Tensor<2,dim> grad_u = ALE_Transformations
			...
			::get_J<dim> (F);
		      ...
		      const Tensor<1,dim> neumann_value
			= ((alpha_u / J) * grad_u * fe_face_values.normal_vector(q)); 
      
		      for (unsigned int i=0; i<dofs_per_cell; ++i) {
            const unsigned int comp_i = fe.system_to_component_index(i).first; 
            if (comp_i == 2 || comp_i == 3)
              {  
                const Tensor<1,dim> phi_i_u = fe_face_values[displacements].value (i, q);
                local_rhs(i) +=  (neumann_value * phi_i_u) * fe_face_values.JxW(q);	   
              }
          // end i
			    }  
		      // end face_n_q_points    
...
// for the whole system. The usual
...
template <int dim>
void
FSI_ALE_Problem<dim>::set_initial_bc ()
{ 
...
template <int dim>
void FSI_ALE_Problem<dim>::newton_iteration () 
...
{ 
  ...
  // Initial i.e. non-homogeneous Dirichlet boundary conditions are 
  // written into the solution vector here. 
  set_initial_bc ();
  assemble_system_rhs();
...
// pressure at the bottom and the current height 
...
// This function evaluates the current distance to the lower
// boundary and the minimal value for J.
template <int dim>
void FSI_ALE_Problem<dim>::compute_minimal_dist_J()
{
  QGauss<dim>   quadrature_formula(parameters.degree+2);
  QGauss<dim-1> face_quadrature_formula(parameters.degree+2);
  FEValues<dim> fe_values (fe, quadrature_formula,
  ...
  std::vector<Vector<double> > solution_face_values (n_face_q_points, 
				 		    Vector<double>(dim+dim+1));
	  ...
	  for (unsigned int face=0; face<GeometryInfo<dim>::faces_per_cell; ++face)
        ...
        if (cell->neighbor_index(face) != -1)	       
          ...
          { 
              ...
              const Tensor<1,dim> u = ALE_Transformations
          ::get_u<dim> (q, solution_face_values);
              const Point<dim> q_point = fe_face_values.quadrature_point(q);
              if (u[1] + q_point[1] < dist_to_bottom)
          dist_to_bottom = u[1] + q_point[1];
          }
         }
	    }
	}      
  ...
  if(min_J < global_min_J){
    global_min_J = min_J;
  }
  std::cout << "Min J: " << time << "   "  << min_J << std::endl;
#if COLLECT_RESULTS
  std::ofstream result_file_J("results-J.txt", std::ios::app);
      result_file_J << std::setw(5) << time << "  "
             << std::scientific << std::setprecision(8) << min_J<< "\n";
      result_file_J.close();
#endif
}

// Here, we determine the spatial mean velocity v*.
template <int dim>
void FSI_ALE_Problem<dim>::compute_v_star()
{
  QGauss<dim>   quadrature_formula(parameters.degree+2);
  FEValues<dim> fe_values (fe, quadrature_formula,
                           update_values    |
  ...
  for (const auto &cell : dof_handler.active_cell_iterators())
      ...
      if (cell->material_id() == 11)
	{
	  for (unsigned int q=0; q<n_q_points; ++q)
	    {
	      const Tensor<1,dim> v = ALE_Transformations
		::get_v<dim> (q, solution_values);
	      v_star += v[1];
        total_n_q_points++;
	    }
	}       
    }
  v_star = v_star / total_n_q_points;
}

// Here, we compute the quantities of interest:
// The pressure at the bottom, the distance to the bottom, 
// the minimal value of J and all other metrics derived from
// these values.
template<int dim>
void FSI_ALE_Problem<dim>::compute_functional_values(const double time)
{
  double p, height_center;
  p = compute_point_value(Point<dim>(0.0,0.0), dim+dim);
  compute_minimal_dist_J();
  height_center = compute_point_value(Point<dim>(0.0,0.05), dim+1);

  std::cout << "------------------" << std::endl;
  std::cout << "Pressure at bottom: " << time << "   " << p << std::endl;
  std::cout << "Distance to bottom: " << time << "   " << dist_to_bottom << std::endl;
  std::cout << "Previous Distance to bottom: " << time << "   " << old_timestep_dist << std::endl;
  std::cout << "------------------" << std::endl;
  if(p > max_pressure){
    max_pressure = p;
  }
  if(dist_to_bottom < min_dist){
    ...
    if(min_dist <= 0.022 && t0_plus_t_star == 0.0){
      t0_plus_t_star = time;
      t_star = t0_plus_t_star - t0;
      compute_v_star();
    }
  }
  if(0.05 + height_center <= 0.039 && t0 == 0.0){
    t0 = time;
  }
  if(dist_to_bottom > old_timestep_dist && t_cont == 0.0 && t0 != 0.0){
    t_cont = time - timestep - t0;
  }
  if(dist_to_bottom < old_timestep_dist && t_cont != 0.0 && t_jump == 0.0){
    t_jump = time - timestep - t0;  
    h_jump = old_timestep_dist;
  }
  old_timestep_dist = dist_to_bottom;
  std::cout << std::endl;
#if COLLECT_RESULTS
      std::ofstream result_file_p("results-p.txt", std::ios::app);
      result_file_p << std::setw(5) << time << "  "
             << std::scientific << std::setprecision(8) << p << "\n";
      result_file_p.close();
      std::ofstream result_file_dist("results-dist.txt", std::ios::app);
      result_file_dist << std::setw(5) << time << "  "
             << std::scientific << std::setprecision(8) << dist_to_bottom << "\n";
      result_file_dist.close();
#endif
}
...
{  
  ...
  // These parameters are read from a parameter file that
  ...
  av_newton_steps = av_newton_steps / (max_no_timesteps + 1);
  std::cout<<"Min distance to bottom: "<<min_dist<<std::endl;
  std::cout<<"Max pressure at bottom: "<<max_pressure<<std::endl;
  std::cout<<"t0: "<<t0<<std::endl;
  #if COLLECT_RESULTS
    std::ofstream result_file_qtoi("results-qtoi.txt", std::ios::app);
      result_file_qtoi << "Maximal pressure at bottom: "
              << std::scientific << std::setprecision(8) << max_pressure << "\n";
      result_file_qtoi << "t0: "
              << std::scientific << std::setprecision(8) << t0 << "\n";
      result_file_qtoi << "t*: "
              << std::scientific << std::setprecision(8) << t_star << "\n";
      result_file_qtoi << "v*: "
              << std::scientific << std::setprecision(8) << v_star << "\n";
      result_file_qtoi << "Time of contact relative to t0: "
              << std::scientific << std::setprecision(8) << t_cont << "\n";
      result_file_qtoi << "Time of highest rebound relative to t0: "
              << std::scientific << std::setprecision(8) << t_jump << "\n";
      result_file_qtoi << "Height of highest rebound: "
              << std::scientific << std::setprecision(8) << h_jump << "\n";
      result_file_qtoi << "Average number of Newton steps: "
              << std::scientific << std::setprecision(8) << av_newton_steps << "\n";
      result_file_qtoi << "Minimal J value over all timesteps: "
              << std::scientific << std::setprecision(8) << global_min_J << "\n";        
      result_file_qtoi.close();
  #endif
}
...
{
  #if COLLECT_RESULTS
      std::ofstream result_file_p("results-p.txt");
      result_file_p.close();
      std::ofstream result_file_J("results-J.txt");
      result_file_J.close();
      std::ofstream result_file_dist("results-dist.txt");
      result_file_dist.close();
      std::ofstream result_file_qtoi("results-qtoi.txt");
      result_file_qtoi.close();
  #endif
  try
    {
      ...
      const unsigned int dim = 2;
	    FSI::FSI_ALE_Problem<dim> flow_problem ("step-fsi_new.prm");      
      flow_problem.run ();
    }
